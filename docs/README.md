# Documentation

## Start here

| Guide | For |
|---|---|
| [first-hour.md](first-hour.md) | Getting value in the first hour |
| [hermes.md](hermes.md) | What Hermes is and how this repository configures it |
| [configuration.md](configuration.md) | Every setting explained |
| [privacy.md](privacy.md) | What stays private and how |

## Working with it

| Guide | For |
|---|---|
| [marketing.md](marketing.md) | The marketing skill system |
| [automation-safety.md](automation-safety.md) | Before you activate any automation |
| [integrations.md](integrations.md) | What connects today versus what does not |
| [mortgage-compliance.md](mortgage-compliance.md) | Boundaries the agent will not cross |
| [troubleshooting.md](troubleshooting.md) | When something breaks |

## Local AI

| Guide | For |
|---|---|
| [local-ai/README.md](local-ai/README.md) | Overview |
| [local-ai/getting-started.md](local-ai/getting-started.md) | Setup, step by step |
| [local-ai/hardware-and-models.md](local-ai/hardware-and-models.md) | Tiers, models, jargon explained |
| [local-ai/privacy-mode.md](local-ai/privacy-mode.md) | What is enforced, and what is not |
| [local-ai/advanced.md](local-ai/advanced.md) | Architecture, llama.cpp, tuning |
| [local-ai/troubleshooting.md](local-ai/troubleshooting.md) | Local AI problems |

## Project

- [../PROJECT_STATUS.md](../PROJECT_STATUS.md) — architecture, decisions, what is tested
- [../local-ai/VALIDATION.md](../local-ai/VALIDATION.md) — exactly what was verified
- [provenance.md](provenance.md) — where the knowledge came from
- [../SECURITY.md](../SECURITY.md) — what must never be committed
